...
| 구분 | 기술 | 선택 이유                                                             |
|------|------|-------------------------------------------------------------------|
| **Backend** | Spring Boot 3.4 | 국내 채용 시장 주류, 검증된 생태계                                              |
| **Database** | PostgreSQL | 좌석 예매의 정합성을 위해 RDB 선택, 비관적 락 활용                                   |
| **Cache/Queue** | Redis | 대기열, 잔여 좌석 캐시, 분산 락, 입장 토큰                                        |
| **분산 락** | Redisson | Redis 기반 분산 락 구현체, Pub/Sub 방식                                     |
| **컨테이너 오케스트레이션** | Kubernetes | 부하테스트로 수평 확장 효과 확인 후, 오토스케일링/무중단 배포/자동 복구를 위해 docker-compose에서 전환 |
